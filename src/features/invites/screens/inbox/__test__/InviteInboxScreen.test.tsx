...
beforeEach(() => {
  jest.clearAllMocks();
  jest.mocked(useInviteInboxScreen).mockReturnValue(makeHookReturn() as any);
...
describe("InviteInboxScreen — ScreenLayout", () => {
...
describe("InviteInboxScreen — InviteList", () => {
  ...
  it("passa invitesByUser como invites ao InviteList", () => {
  ...
  it("passa fetchInvitesByUser como onRetry ao InviteList", () => {
    render(<InviteInboxScreen />);
    const props = jest.mocked(InviteList).mock.calls[0][0] as any;
    expect(props.onRetry).toBe(mockFetchInvitesByUser);
  });

  it("passa handleAcceptInvite e handleRejectInvite ao InviteList", () => {
    render(<InviteInboxScreen />);
    const props = jest.mocked(InviteList).mock.calls[0][0] as any;
  ...
  it("passa variant='received' ao InviteList", () => {
    render(<InviteInboxScreen />);
    const props = jest.mocked(InviteList).mock.calls[0][0] as any;
    expect(props.variant).toBe("received");
  });

  it("emptyState onPress chama router.push para a rota de perfil", () => {
    render(<InviteInboxScreen />);
    const props = jest.mocked(InviteList).mock.calls[0][0] as any;
